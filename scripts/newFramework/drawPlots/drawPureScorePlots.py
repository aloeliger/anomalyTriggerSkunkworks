...
def main(args):
    ...
    for run in runs:
        ...
        for menu in menus[run]:
            ...
            pureVsOverallCurve.SetTitle(f";CICADA Overall Rate (kHz); CICADA Pure Rate (kHz)")
            pureVsOverallCurve.SetMarkerStyle(7)
            pureVsOverallCurve.SetMarkerSize(4)
            pureVsOverallCurve.SetMarkerColor(ROOT.kRed)
            pureVsOverallCurve.SetLineColor(ROOT.kRed)
            pureVsOverallCurve.SetLineWidth(2)
            pureVsOverallCanvas = ROOT.TCanvas(f'{run}_{menu}_p_Vs_o',f'{run}_{menu}_p_Vs_o',1400,1000)
            ...
            rightLatex.SetTextSize(0.04)
            rightLatex.SetNDC(True)
            rightLatex.SetTextAlign(31)
            # rightLatex.DrawLatex(0.9, 0.92, "#font[52]{"+menu+"}")
